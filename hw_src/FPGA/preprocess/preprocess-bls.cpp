/*
   Adapted with small modification, Amin Sahebi, 2020-2021, University of Florence,
...
void generate_edge_grid(std::string input, std::string output,
	...
	for (int ti = 0; ti < parallelism; ti++) {
		threads.emplace_back([&]() {
				...
				char *buffer = buffers[cursor];
				for (long pos = 0; pos < bytes; pos += edge_unit) {
				source = advanced ? mapper[*(VertexId *)(buffer + pos)]
				: *(VertexId *)(buffer + pos);
				target = advanced
	...
	fprintf(fmeta, "%d %d %ld %d %d", edge_type, vertices, edges, partitions,
			advanced ? 1 : 0);
	fclose(fmeta);
}
...
int main(int argc, char **argv) {
	...
	if (input == "" || output == "" || vertices == -1 ||
		...
		fprintf(stderr,
				...
				"[partitions] -t [edge type: 0=unweighted, 1=weighted] -m [mode: "
				"0=advanced, 1=naive]\n",
				argv[0]);
		exit(-1);
	}
