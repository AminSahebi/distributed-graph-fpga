...
# 2 "sssp_kernel.cpp" 2
# 1 "/tools/Xilinx/Vitis_HLS/2021.1/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/stdint.h" 1 3
# 63 "/tools/Xilinx/Vitis_HLS/2021.1/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/stdint.h" 3
...
# 64 "/tools/Xilinx/Vitis_HLS/2021.1/lnx64/tools/clang-3.9-csynth/lib/clang/7.0.0/include/stdint.h" 2 3
# 3 "sssp_kernel.cpp" 2
...
void PE_kernel(u32 local_in_a[], u32 local_in_b[], u32 local_in_c[], u32 local_out[]) {
    u32 distance_buffer[4096/128];
#pragma HLS ARRAY_PARTITION variable=distance_buffer dim=0


init_distances:
    for (int j = 0; j < 4096/128; j++) {
#pragma HLS pipeline
 distance_buffer[j] = (local_in_a[j] == 0) ? 0 : (4294967295U);
    }


sssp_computation:
    for (int k = 0; k < v - 1; k++) {
        VITIS_LOOP_33_1: for (int j = 0; j < 4096/128; j++) {
#pragma HLS pipeline
 u32 src = local_in_a[j];
            u32 dst = local_in_b[j];
            u32 weight = local_in_c[j];

            if (distance_buffer[src] != (4294967295U) && distance_buffer[src] + weight < distance_buffer[dst]) {
...
void buffer_load(u32 local_in_a[128][4096/128], u32 local_in_b[128][4096/128], u32 local_in_c[128][4096/128], u_data *global_in_a, u_data *global_in_b, u_data *global_in_c) {
    VITIS_LOOP_54_1: for (int i = 0; i < 128; i++) {
#pragma HLS pipeline
 memcpy(local_in_a[i], &global_in_a[i * 4096/128], 4096/128 * sizeof(u32));
        memcpy(local_in_b[i], &global_in_b[i * 4096/128], 4096/128 * sizeof(u32));
        memcpy(local_in_c[i], &global_in_c[i * 4096/128], 4096/128 * sizeof(u32));
    }
}

void buffer_compute(u32 local_in_a[128][4096/128], u32 local_in_b[128][4096/128], u32 local_in_c[128][4096/128], u32 local_out[128][4096/128]) {
    VITIS_LOOP_63_1: for (int i = 0; i < 128; i++) {
#pragma HLS UNROLL
 PE_kernel(local_in_a[i], local_in_b[i], local_in_c[i], local_out[i]);
    }
}

void buffer_store(u_data *global_out, u32 local_out[128][4096/128]) {
    VITIS_LOOP_70_1: for (int i = 0; i < 128; i++) {
#pragma HLS pipeline
 memcpy(&global_out[i * 4096/128], local_out[i], 4096/128 * sizeof(u32));
    }
}

extern "C" {
    __attribute__((sdx_kernel("sssp_kernel_0", 0))) void sssp_kernel_0(
        u_data *e_src,
        u_data *e_dst,
        u_data *out_degree,
        u_data *out_r,
...
#pragma HLS TOP name=sssp_kernel_0
# 84 "sssp_kernel.cpp"
...
#pragma HLS INTERFACE m_axi port = out_r offset = slave bundle=gmem num_write_outstanding=64 max_write_burst_length=64 num_read_outstanding=64 max_read_burst_length=64
 ...
 v = vertices;
        ...
        u32 e_src_buffer_a[128][4096/128];
        u32 e_src_buffer_b[128][4096/128];
        u32 e_dst_buffer_a[128][4096/128];
        u32 e_dst_buffer_b[128][4096/128];
        u32 out_deg_buffer_a[128][4096/128];
        u32 out_deg_buffer_b[128][4096/128];
        u32 output_buffer_a[128][4096/128];
  u32 output_buffer_b[128][4096/128];
#pragma HLS ARRAY_PARTITION variable=output_buffer_b complete dim=1
 u32 local_out[128][4096/128];
        u32 local_in_a[128][4096/128];
        u32 local_in_b[128][4096/128];
        u32 local_in_c[128][4096/128];

        VITIS_LOOP_106_1: for (int i = 0; i < size / 4096 + 1; i++) {

            if (i % 2 == 0) {
                buffer_load(e_src_buffer_a, e_dst_buffer_a, out_deg_buffer_a, e_src + i * 4096/128, e_dst + i * 4096/128, out_degree + i * 4096/128);
                buffer_compute(e_src_buffer_b, e_dst_buffer_b, out_deg_buffer_b, output_buffer_b);
                buffer_store(out_r + i * 4096/128, output_buffer_a);
            } else {
                buffer_load(e_src_buffer_b, e_dst_buffer_b, out_deg_buffer_b, e_src + i * 4096/128, e_dst + i * 4096/128, out_degree + i * 4096/128);
                buffer_compute(e_src_buffer_a, e_dst_buffer_a, out_deg_buffer_a, output_buffer_a);
                buffer_store(out_r + i * 4096/128, output_buffer_b);
            }
        }
    }
}
