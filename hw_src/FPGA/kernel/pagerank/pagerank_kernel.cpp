...
struct CacheBlock {
	CacheEntry entries[2];  // 2 entries
};
...
#pragma HLS pipeline II=1
		u32 src1 = read_from_cache(cache_L1, j);//local_in_a[j];
		u32 deg1 = local_in_b[j];
		u32 temp1 = src1 / deg1;
		u32 rank1 = adding_constant + DAMPING_FACTOR * temp1;
		//rank_buffer[j] = rank1;
		write_to_cache(cache_L1, j, rank1);

		u32 src2 = read_from_cache(cache_L1, j+1);//local_in_a[j + 1];
		u32 deg2 = local_in_b[j + 1];
		u32 temp2 = src2 / deg2;
		u32 rank2 = adding_constant + DAMPING_FACTOR * temp2;
		write_to_cache(cache_L1, j+1, rank1);//rank_buffer[j + 1] = rank2;
	}
