...
void PE_kernel(u32 local_in_a[], u32 local_in_b[], u32 local_in_c[], u32 local_out[]) {
	...
	u32 prev_buffer[PE][BUF_PER_PE]; // Local Memory to store result
#pragma HLS ARRAY_PARTITION variable=prev_buffer dim=1 cyclic=PE
	u32 rank_buffer[PE][BUF_PER_PE]; // Local Memory to store result
#pragma HLS ARRAY_PARTITION variable=rank_buffer dim=1 cyclic=PE
...
kernel_loop:	for(int i=0 ; i < PE; i++){
			for(int j = 0; j < BUF_PER_PE; j++){
#pragma HLS pipeline
				prev_buffer[i][j] = one_over_n;
				rank_buffer[i][j] = 0.0;
				src = local_in_a[j]; //edge src buffer
				dst = local_in_b[j]; //edge dst buffer
				u32 temp = prev_buffer[i][src]/local_in_c[src];
				rank_buffer[i][dst] = temp;
				local_out[dst] = rank_buffer[i][dst];
			}
		}
}
