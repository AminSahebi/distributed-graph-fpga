...
#define foreach_outgoing_edges2(b, _dst)                                       \
    for (size_t ___j = 0, ___size = n->nb_out_edges;                           \
         (___size <= NB_MAX_EDGES &&                                           \
          ___j <                                                               \
              ___size) /*|| unlock_edge_buffer( &n->outgoing_edges, ___max)*/; \
         ___j++)                                                               \
        for (size_t ___k = 0; (___size <= NB_MAX_EDGES && ___k < 1); ___k++)   \
            if (___size <= NB_MAX_EDGES &&                                     \
                (/*n->outgoing_edges.edges[___j]*/ edge_array_out              \
                     [n->outgoing_edges + ___j]                                \
                         .dst < NB_NODES) &&                                   \
                (_dst =                                                        \
                     &nodes[edge_array_out[n->outgoing_edges + ___j]           \
                                .dst /*n->outgoing_edges.edges[___j].dst*/]))

#define foreach_outgoing_edges3(b, _dst, e)                                   \
    for (size_t ___j = 0,                                                     \
                /*___max = lock_edge_buffer( &n->outgoing_edges),*/ ___size = \
                    n->nb_out_edges;                                          \
         /*outgoing_edges.nb_edges;*/ (                                       \
             ___size <= NB_MAX_EDGES &&                                       \
             ___j < ___size) /* || unlock_edge_buffer( &n->outgoing_edges,    \
                                ___max) */                                    \
         ;                                                                    \
         ___j++)                                                              \
        for (size_t ___k = 0; (___size <= NB_MAX_EDGES && ___k < 1); ___k++)  \
            if (___size <= NB_MAX_EDGES &&                                    \
                (edge_array_out[n->outgoing_edges + ___j].dst < NB_NODES) &&  \
                (_dst =                                                       \
                     &nodes[edge_array_out[n->outgoing_edges + ___j].dst]) && \
                (e = &edge_array_out[n->outgoing_edges + ___j]))

#define foreach_outgoing_edges(...)                                          \
    GET_MACRO(__VA_ARGS__, foreach_outgoing_edges3, foreach_outgoing_edges2) \
    (__VA_ARGS__)

#define foreach_incoming_edges2(b, _dst)                                       \
    for (size_t ___j = 0,                                                      \
                /* ___max = lock_edge_buffer(&n->incoming_edges), */ ___size = \
                    n->nb_in_edges;                                            \
         (___size <= NB_MAX_EDGES &&                                           \
          ___j < ___size) /*  || unlock_edge_buffer(&n->incoming_edges,        \
                             ___max) */                                        \
         ;                                                                     \
         ___j++)                                                               \
        for (size_t ___k = 0; (___size <= NB_MAX_EDGES && ___k < 1); ___k++)   \
            if (___size <= NB_MAX_EDGES &&                                     \
                (edge_array_in[n->incoming_edges + ___j].dst < NB_NODES) &&    \
                (_dst = &nodes[edge_array_in[n->incoming_edges + ___j].dst]))
...
#define foreach_incoming_edges3(b, _dst, e)                                    \
    for (size_t ___j = 0,                                                      \
         ...
         (___size <= NB_MAX_EDGES &&                                           \
          ___j <                                                               \
              ___size) /*|| unlock_edge_buffer(&n->incoming_edges, ___max)*/;  \
         ___j++)                                                               \
        for (size_t ___k = 0; (___size <= NB_MAX_EDGES && ___k < 1); ___k++)   \
            if (___size <= NB_MAX_EDGES &&                                     \
                (edge_array_in[n->incoming_edges + ___j].dst < NB_NODES) &&    \
                (_dst =                                                        \
                     &nodes[edge_array_in[n->incoming_edges + ___j].dst]) &&   \
                (e = &edge_array_in[n->incoming_edges + ___j]))

#define foreach_incoming_edges(...)                                          \
    GET_MACRO(__VA_ARGS__, foreach_incoming_edges3, foreach_incoming_edges2) \
    (__VA_ARGS__)
