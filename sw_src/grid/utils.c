...
int sort_edges_dst(const void *a, const void *b) {
    const struct edge *const e1 = (const struct edge *)a,
                             *const e2 = (const struct edge *)b;
    if (e1->dst > e2->dst) return 1;
    if (e1->dst < e2->dst) return -1;
    return 0;
}
...
struct node_list get_all_nodes(void) {
    ...
    uint64_t *starting_nodes =
        (uint64_t *)malloc(NB_NODES * sizeof(*starting_nodes));
#pragma omp parallel for
    for (i = 0; i < NB_NODES; i++) starting_nodes[i] = i;
    nb_starting_nodes = NB_NODES;
    struct node_list n = {.nb_starting_nodes = nb_starting_nodes,
                          .starting_nodes = starting_nodes};
    return n;
}

int sort_edges(const void *_a, const void *_b) {
    const struct edge *a = (const struct edge *)_a,
                      *b = (const struct edge *)_b;
    if (a->dst > b->dst)
        return 1;
    else if (a->dst < b->dst)
        return -1;
